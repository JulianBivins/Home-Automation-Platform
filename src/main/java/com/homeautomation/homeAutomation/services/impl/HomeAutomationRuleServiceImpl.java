...
//    }
    ...
    public boolean isOwner(Long ruleId, Long deviceId, String currentUsername) {
        Optional<HomeAutomationRuleEntity> rule = homeAutomationRuleRepository.findById(ruleId);
        if (rule.isEmpty()) {
            System.err.println("Rule ID not found: " + ruleId);
            return false;
        }

        if (!rule.get().getUserEntity().getUsername().equals(currentUsername)) {
            System.err.println("User : " + currentUsername + "does not own Rule with ID: " + ruleId);
            return false;
        }

        Optional<DeviceEntity> device = deviceRepository.findById(deviceId);
        if (device.isEmpty()) {
            System.err.println("Device not found with ID : " + deviceId);
            return false;
        }

        boolean isAssociated = device.get().getRules().contains(rule.get());
        if (!isAssociated) {
            System.err.println("Device with ID : "  + deviceId + " is not associated to rule with ID : " + ruleId);
        }
